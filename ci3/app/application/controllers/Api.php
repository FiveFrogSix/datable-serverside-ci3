...
class Api extends CI_Controller {
	...
	public function insertLoop(){
		for($i=1; $i<=5556; $i++){
			...
			$this->db->insert('employee_fake', $array);
		}
	}
	...
	public function serverside2(){
		...
		');
		$this->db->from('employee_fake');
		$this->db->like('name',$_GET['search']['value'],'right');
		$this->db->or_like('department',$_GET['search']['value'],'right');
		if(intval($_GET['search']['value'])){
			$this->db->or_where('salary >=',$_GET['search']['value'],'right');
		}
